...
public class AppP1 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        getCorrectResponse(validUrls, method, url, headers, bodyParams, response);

        System.out.println(response.toString());

    }

    private static void getCorrectResponse(List<String> validUrls, String method, String url, Map<String, String> headers, Map<String, String> bodyParams, StringBuilder response) {
                    ...
                    .append(String
                            .format("Greetings %s!",
                                    new String(Base64.getDecoder().decode
                                            (headers.get("Authorization").split("\\s+")[1]))
                            ));
            ...
            response
                    ...
                    .append(String
                            .format("Greetings %s! You have successfully created %s with %s - %s, %s - %s.",
                                    new String(Base64.getDecoder().decode
                                            (headers.get("Authorization").split("\\s+")[1])),
                                    paramValues.get(0), paramNames.get(1), paramValues.get(1),
                                    paramNames.get(2), paramValues.get(2)
                            ));
        }
    }
    ...
    private static Map<String, String> getBodyParams(List<String> request) {
        ...
        Arrays.stream(request.get(request.size() - 1)
                ...
                .map(bp -> bp.split("="))
                .forEach(bpKvp -> {
                    bodyParams.put(bpKvp[0], bpKvp[1]);
                });
